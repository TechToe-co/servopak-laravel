
<div class="bg-black text-white">
    <div class="container xl:px-40 px-5 mx-auto py-20">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-20">
            <div class="md:col-span-1 lg:col-span-4">
                <a class="text-4xl font-bold flex gap-2" href="#">
                    <img class="w-12 h-10 pe-2" src="img/logo.png" alt="" />
                    TransitFlow
                </a>
                <p class="pt-5 text-gray kumbh">
                    TransitFlow is dedicated to revolutionizing urban mobility by
                    offering smart, sustainable, and efficient transit solutions. Our
                    mission is to enhance the way people navigate cities.
                </p>
            </div>
            <div class="md:col-span-1 lg:col-span-2">
                <p class="text-xl font-bold pb-7 fira">Our Company</p>
                <ul>
                    <li class="pb-2">
                        <a class="text-gray hover:text-yellow transition-all duration-300 hover:underline"
                            href="#">Home</a>
                    </li>
                    <li class="pb-2">
                        <a class="text-gray hover:text-yellow transition-all duration-300 hover:underline"
                            href="#">About</a>
                    </li>
                    <li class="pb-2">
                        <a class="text-gray hover:text-yellow transition-all duration-300 hover:underline"
                            href="#">Team</a>
                    </li>
                    <li class="">
                        <a class="text-gray hover:text-yellow transition-all duration-300 hover:underline"
                            href="#">Service</a>
                    </li>
                </ul>
            </div>
            <div class="md:col-span-1 lg:col-span-3">
                <p class="text-xl font-bold pb-7 fira">Contact Us</p>
                <ul>
                    <li class="pb-2">
                        <a class="text-gray hover:text-yellow transition-all duration-300 hover:underline"
                            href="#">Email:taimour@servopak.co</a>
                    </li>
                    <li class="pb-2">
                        <a class="text-gray hover:text-yellow transition-all duration-300 hover:underline"
                            href="#">Phone:917-325-3600</a>
                    </li>
                </ul>
            </div>
            <div class="md:col-span-1 lg:col-span-3">
                <p class="text-xl font-bold pb-7 fira">Our Address</p>
                <p class="text-gray">
                    Apt T1 6626 Ronald Rd MD Capitol Heights 20743
                </p>
                <div class="pt-5">
                    <a class="text-yellow underline font-semibold hover:text-white transition-all duration-300"
                        href="#">View all location</a>
                </div>
            </div>
        </div>
        <div class="border-b border-gray pt-20"></div>
        <div class="grid grid-cols-1 lg:grid-cols-2 pt-10">
            <div>
                <p>Copyright © 2024 TransitFlow. All Rights Reserved.</p>
            </div>
            <div class="g:text-end mt-5 pt-5 lg:mt-[-20px]">
                <a class="hover:text-yellow transition-all duration-300 mr-8" href="#">Terms & Conditions</a>
                <a class="hover:text-yellow transition-all duration-300" href="#">Terms & Conditions</a>
            </div>
        </div>
    </div>
</div>
